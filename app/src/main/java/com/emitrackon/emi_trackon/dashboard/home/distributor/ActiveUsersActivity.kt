package com.emitrackon.emi_trackon.dashboard.home.distributor
...
import com.emitrackon.emi_trackon.dashboard.people.retailer.Retailer
import com.emitrackon.emi_trackon.dashboard.people.retailer.RetailerViewActivity
import com.emitrackon.emi_trackon.databinding.ActivityActiveUsersBinding
...
class ActiveUsersActivity : AppCompatActivity() {
    private val binding by lazy { ActivityActiveUsersBinding.inflate(layoutInflater) }
    private lateinit var adapter: DistributorActiveUserAdapter
    val list = ArrayList<Dashboard.ActiveCostomer>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter = DistributorActiveUserAdapter(list, this) { data, action, pos ->
            when (action) {
                "action" -> {
                    callChangeStatusApi(
                        data.id.toString(),
    ...
    private fun filterList(key: String) {
        val filter_list = ArrayList<Dashboard.ActiveCostomer>()
        list.forEach {
    ...
    private fun callApi() {
        binding.pb.show()
        val call = RetrofitInstance.apiService.getActiveDistributorListApi()
        call.enqueue(object : Callback<Dashboard.ActiveDistributorListResponse> {
            override fun onResponse(
                call: Call<Dashboard.ActiveDistributorListResponse>,
                response: Response<Dashboard.ActiveDistributorListResponse>
            ) {
                binding.pb.hide()
                when (response.code()) {
                    200 -> {
